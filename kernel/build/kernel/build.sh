...
if [[ -z "${SKIP_EXT_MODULES}" ]] && [[ -n "${EXT_MODULES}" ]]; then
  ...
  MAKE_ARGS+=("CHIPSET_COMPANY=${CHIPSET_COMPANY}")
  MAKE_ARGS+=("OPLUS_VND_BUILD_PLATFORM=${OPLUS_VND_BUILD_PLATFORM}")

  # setup out-of-tree ko conditional compilation parameters
  if [ "${TARGET_BUILD_VARIANT}" == "userdebug" ]; then
    echo "++++++userdebug+++++++"
    MAKE_ARGS+=("OPLUS_FEATURE_BSP_DRV_VND_INJECT_TEST=1")
  elif [ "${TARGET_BUILD_VARIANT}" == "debug" ]; then
    echo "++++++debug+++++++"
    MAKE_ARGS+=("OPLUS_FEATURE_BSP_DRV_VND_INJECT_TEST=1")
  elif [ "${OPLUS_FEATURE_BSP_DRV_VND_INJECT_TEST}" == "1" ]; then
    echo "++++++OPLUS_FEATURE_BSP_DRV_VND_INJECT_TEST=1+++++++"
    MAKE_ARGS+=("OPLUS_FEATURE_BSP_DRV_VND_INJECT_TEST=1")
  else
    echo "++++++OPLUS_FEATURE_BSP_DRV_VND_INJECT_TEST disabled+++++++"
  fi
