...
void kbase_context_sticky_resource_term(struct kbase_context *kctx)
{
	...
	while (pending_regions_to_clean) {
		...
		if (cookie > 0 && !WARN_ON(!kctx->pending_regions[cookie])) {
			...
			if(kctx->pending_regions[cookie]->cpu_alloc)
				kbase_mem_phy_alloc_put(
						kctx->pending_regions[cookie]->cpu_alloc);
			else
				dev_err(kctx->kbdev->dev, "ctx %d_%d cpu_alloc is NULL flags %lx alloc %d\n",
						kctx->tgid, kctx->id, kctx->pending_regions[cookie]->flags, kctx->pending_regions[cookie]->cpu_alloc != kctx->pending_regions[cookie]->gpu_alloc);

			if(kctx->pending_regions[cookie]->gpu_alloc)
				kbase_mem_phy_alloc_put(
						kctx->pending_regions[cookie]->gpu_alloc);
			else
				dev_err(kctx->kbdev->dev, "ctx %d_%d gpu_alloc is NULL flags %lx alloc %d\n",
						kctx->tgid, kctx->id, kctx->pending_regions[cookie]->flags, kctx->pending_regions[cookie]->cpu_alloc != kctx->pending_regions[cookie]->gpu_alloc);
