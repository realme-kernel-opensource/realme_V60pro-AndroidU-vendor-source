...
static int kbase_csf_reset_gpu_now(struct kbase_device *kbdev, bool firmware_inited, bool silent)
{
	...
	/* set the csg terminate timeout back to orignal */
...
static void kbase_csf_reset_gpu_worker(struct work_struct *data)
{
...
#if IS_ENABLED(CONFIG_MALI_MTK_TIMEOUT_REDUCE)
	/* set csg suspend timeout to 1.0s after GPU reset */
	kbdev->csf.csg_suspend_timeout_ms =
		kbase_get_timeout_ms(kbdev, CSF_CSG_SUSPEND_TIMEOUT_AFTER_ABNORMAL_TIMEOUT);
	/* set csg term timeout to 0.5s after GPU reset */
	kbdev->csf.csg_term_timeout_ms =
		kbase_get_timeout_ms(kbdev, CSF_CSG_TERM_TIMEOUT_AFTER_ABNORMAL_TIMEOUT);
#endif /* CONFIG_MALI_MTK_TIMEOUT_REDUCE */
	...
	KBASE_KTRACE_ADD(kbdev, DUMP_END, NULL, 0);
	if(end.tv_sec - start.tv_sec >= 5) {
		/* dump the ktrace after completion of reset */
		KBASE_KTRACE_DUMP(kbdev);
	} else {
		dev_warn(kbdev->dev, "GPU reset takes less than 5s");
	}
}
...
void kbase_reset_gpu(struct kbase_device *kbdev)
{
	...
	KBASE_KTRACE_ADD(kbdev, DUMP_BEGIN, NULL, 0);
