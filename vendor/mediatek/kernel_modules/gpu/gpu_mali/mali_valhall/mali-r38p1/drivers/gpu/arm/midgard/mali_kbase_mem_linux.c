					...
					enum kbase_caller_mmu_sync_info mmu_sync_info)
{
	...
	if (*flags & BASE_MEM_SAME_VA) {
		...
		if (bitmap_empty(kctx->cookies, BITS_PER_LONG)) {
			dev_err(dev, "ctx %d_%d No cookies available for allocation!", kctx->tgid, kctx->id);
			dump_stack();
			kbase_gpu_vm_unlock_with_pmode_sync(kctx);
			goto no_cookie;
		}
		    ...
		    u64 *num_pages)
{
	...
	*num_pages = nents * stride;
	...
	if (!kbase_ctx_flag(kctx, KCTX_COMPAT)) {
		/* Bind to a cookie */
		if (bitmap_empty(kctx->cookies, BITS_PER_LONG)) {
			dev_err(kctx->kbdev->dev, "ctx %d_%d No cookies available for allocation!", kctx->tgid, kctx->id);
			dump_stack();
			goto no_cookie;
		}
		/* return a cookie */
...

{
	...
	*aligned_offset = 0;
	...
	if (!reg) {
		err = -ENOMEM;
		dev_err(kctx->kbdev->dev, "ctx %d_%d cookie %d %s:%d\n", kctx->tgid, kctx->id, cookie, __FUNCTION__, __LINE__);
		goto out;
	}
