	...
	mutex_lock(&kbdev->kctx_list_lock);
	{
		...
		list_for_each_entry(kctx, &kbdev->kctx_list, kctx_list_link) {
			if (kctx->tgid == pid) {
				...
				ret = wait_for_completion_timeout(&kctx->csf.cpu_queue.dump_cmp, msecs_to_jiffies(1000));
				if (!ret) {
					dev_info(kbdev->dev, "[cpu_queue] timeout for Ctx %d_%d CPU Queue dump completion (1000 ms)", kctx->tgid, kctx->id);
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
					mtk_logbuffer_type_print(kbdev, MTK_LOGBUFFER_TYPE_ALL,
						"[cpu_queue] timeout for Ctx %d_%d CPU Queue dump completion (1000 ms)\n",
						kctx->tgid, kctx->id);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
					mutex_unlock(&kbdev->kctx_list_lock);
					return;
