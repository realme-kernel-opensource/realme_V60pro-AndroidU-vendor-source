...
static int oplus_chg_vc_wd_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	struct oplus_virtual_cp_ic *vc;
	int i;
	int rc = 0;
	...
	vc = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vc->child_num; i++) {
		rc = oplus_chg_ic_func(vc->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_WATCHDOG_ENABLE, en);
		if (rc < 0 && rc != -ENOTSUPP) {
			chg_err("child ic[%d] %s error, rc=%d\n", i, en ? "enable" : "disable", rc);
			/* TODO: need push ic error msg, and offline child ic */
			return rc;
