...
#define oplus_chg_ic_func(ic, func_id, ...) ({				\
	func_id##_T _func =						\
		ic ? oplus_chg_ic_debug_get_func(ic, func_id) :	NULL;	\
...
#define oplus_print_ic_err(msg)					\
	chg_err("[IC_ERR][%s]-[%s]:%s\n", msg->ic->name,	\
		oplus_chg_ic_err_str(msg->type), msg->msg);

/* This file is automatically generated */
#include "oplus_chg_ic_cfg.h"

#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
#define OPLUS_CHG_IC_DEBUG_DATA_TYPE	s64
#define OPLUS_CHG_IC_FUNC_DATA_MAGIC	0x20302009
#define OPLUS_CHG_IC_WATCHDOG_TIME_MS	1000
struct oplus_chg_ic_func_arg_item {
	u32 num;
	u32 size;
	u32 str_data;
	u8 buf[0];
...
struct oplus_chg_ic_virt_data {
	enum oplus_chg_ic_func func_id;
	struct list_head list;
};

struct oplus_chg_ic_func_data {
	enum oplus_chg_ic_func func_id;
	u32 trace_only;
	u32 size;
	u32 pid;
...
struct oplus_chg_ic_debug_data {
	...
	struct list_head virt_list;
	struct mutex overwrite_list_lock;
	struct mutex virt_list_lock;
...
struct oplus_chg_ic_dev {
	...
	struct mutex func_data_lock;
	struct mutex func_call_lock;
	struct completion func_data_ack;
	bool func_data_ok;
#endif /* CONFIG_OPLUS_CHG_IC_DEBUG */
};
