...
static ssize_t adapter_power_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (chip->wls_online) {
		...
		if (chip->ufcs_oplus_adapter)
			power = oplus_ufcs_adapter_id_to_power(chip->ufcs_adapter_id) * 1000;
		else
			power = oplus_ufcs_adapter_id_to_power(UFCS_FASTCHG_TYPE_THIRD) * 1000;
	} else 	if (chip->pps_online) {
		if (chip->pps_oplus_adapter)
		...
		power = sid_to_adapter_power(chip->vooc_sid) * 1000;
	} else {
		switch (chip->wired_type) {
		case OPLUS_CHG_USB_TYPE_QC2:
		case OPLUS_CHG_USB_TYPE_QC3:
