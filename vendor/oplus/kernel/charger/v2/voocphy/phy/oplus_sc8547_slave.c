...
static int sc8547_slave_cp_wd_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	struct sc8547a_slave_device *chip;
	int ret = 0;
	...
	chg_info("set sc8547_slave_cp_wd_enable (%d)\n", en);

	if (en)
		ret = sc8547_slave_update_bits(chip->voocphy->slave_client, SC8547_REG_09,
		                               SC8547_WATCHDOG_MASK, SC8547_WATCHDOG_1S); /* WD:1000ms */
	else
		ret = sc8547_slave_update_bits(chip->voocphy->slave_client, SC8547_REG_09,
		                               SC8547_WATCHDOG_MASK, SC8547_WATCHDOG_DIS); /* dsiable wdt */

	if (ret < 0) {
		chg_err("failed to set sc8547_slave_cp_wd_enable (%d)(%d)\n", en, ret);
		return ret;
	}
