...
static void nu1669_disable_standby(struct oplus_nu1669 *chip)
{
	int rc;
...
static int nu1669_set_rx_mode(struct oplus_chg_ic_dev *dev, enum oplus_chg_wls_rx_mode rx_mode)
{
	...
	rc = nu1669_set_rx_enable(dev, false);
	if (rc < 0)
		chg_err("set rx disable failed, rc=%d\n", rc);

	msleep(100);

	rc = nu1669_set_rx_enable(dev, true);
	if (rc < 0)
...
static int nu1669_set_fod_parm(struct oplus_chg_ic_dev *dev, u8 data[], int len)
{
	return 0;
}

static int nu1669_send_msg(struct oplus_chg_ic_dev *dev, unsigned char msg[], int len)
{
	struct oplus_nu1669 *chip;
	int rc;
...
static void nu1669_event_process(struct oplus_nu1669 *chip)
{
	...
	if (!enable && nu1669_get_wls_type(chip) != OPLUS_CHG_WLS_TRX) {
...
static void nu1669_event_work(struct work_struct *work)
{
	...
	if (chip->rx_connected == true || nu1669_get_wls_type(chip) == OPLUS_CHG_WLS_TRX)
...
static void nu1669_shutdown(struct i2c_client *client)
{
	...
	if (is_connected &&
		...
		nu1669_set_rx_enable(chip->ic_dev, false);
		msleep(100);
