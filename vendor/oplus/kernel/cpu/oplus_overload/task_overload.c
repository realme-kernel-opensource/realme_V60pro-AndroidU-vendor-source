...
bool check_abnormal_cpu_util(void)
{
	...
	for (i = 0; i < golden_cpu; i++) {
		unsigned long golden_cpu_limit_capacity = limited_capacity(golden_cpu_first + i) * GOLD_CAP_THRESHOLD / CPU_CAP;
		goden_sum += cpu_util(golden_cpu_first + i);
		goden_cap += golden_cpu_limit_capacity;
		if (cpu_util(golden_cpu_first + i) < golden_cpu_limit_capacity)
			goden++;
	}
