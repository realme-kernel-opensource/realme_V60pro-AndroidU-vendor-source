...
int dump_procs_fd_info(struct files_acct *acct, const char *title)
{
	...
	for_each_process(tsk) {
		...
		if (PAGES(acct->sz) > acct->watermark) {
			acct->show = true;
			 /* iter fd twice & show buf info without zero acct.size */
			task_lock(tsk);
			iterate_fd(tsk->files, 0, acct->iter_fd, (void *)acct);
			task_unlock(tsk);
		}
...
static void __lowmem_dbg_dump(struct lowmem_dbg_cfg *cfg)
{
	...
	dump_procs_fd_info(&files_acct, "dump_procs_ashmem_info");
