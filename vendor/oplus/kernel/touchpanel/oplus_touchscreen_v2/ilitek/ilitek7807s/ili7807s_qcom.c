...
int ili_sleep_handler(int mode)
{
	...
	case TP_SUSPEND:
		...
		if (ilits->ts->sportify_aod_gesture_support) {
			ilits->aod_in = 0;
...
{
	struct ilitek_ts_data *chip_info = (struct ilitek_ts_data *)chip_data;
	if (points == NULL || chip_info == NULL) {
		return -EINVAL;
	}
	if (max_num < MAX_TOUCH_NUM) {
		return -EINVAL;
	}
...
static int ilitek_mode_switch(void *chip_data, work_mode mode, int flag)
{
	...
	case MODE_GESTURE:
		...
		if (flag) {
			...
			} else if (ilits->aod_in && ilits->ts->sportify_aod_gesture_support) {
				ili_sleep_handler(TP_SUSPEND);
			} else {
	...
	case MODE_EDGE:
		if (ilits->aod_in && ilits->ts->sportify_aod_gesture_support) {
			ILI_DBG("aod state not set MODE_EDGE\n");
			break;
		}
