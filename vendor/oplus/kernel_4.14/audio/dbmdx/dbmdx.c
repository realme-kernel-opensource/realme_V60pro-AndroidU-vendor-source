...
static int dbmdx_common_probe(struct dbmdx_private *p)
{
...
#ifdef CONFIG_OF
	/* 1.1V LDO */
	p->pdata->gpio_ldo = of_get_named_gpio(np, "ldo-gpio", 0);
#endif
	if (!gpio_is_valid(p->pdata->gpio_ldo)) {
		dev_err(p->dev, "%s: reset gpio invalid\n", __func__);
		ret = -EINVAL;
		goto err_clk_off;
	}

	ret = gpio_request(p->pdata->gpio_ldo, "DBMDX ldo");
	if (ret < 0) {
		dev_err(p->dev, "%s: error requesting reset gpio\n", __func__);
		goto err_clk_off;
	}
	gpio_direction_output(p->pdata->gpio_ldo, 1);
	gpio_set_value(p->pdata->gpio_ldo, 1);
	dev_err(p->dev, "%s: set ldo enable\n", __func__);
	...
	dev_err(p->dev, "%s: reset gpio:%d\n", __func__,p->pdata->gpio_reset);
...
#ifdef OPLUS_ARCH_EXTENDS
	/* Yongzhi.Zhang@PSW.MM.AudioDriver.Machine, 2018/11/14, add dbmdx */
	ret = pmic_set_register_value(PMIC_RG_RTC32K_1V8_1_PDN, 0x0);
	pr_info("%s: DBMDX enable rtc32k-0 once ret = %d\n", __func__, ret);
#endif /* OPLUS_ARCH_EXTENDS */
