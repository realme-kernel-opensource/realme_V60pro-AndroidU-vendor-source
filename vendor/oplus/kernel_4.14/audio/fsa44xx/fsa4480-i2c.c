...
//#endif /*OPLUS_ARCH_EXTENDS*/
	...
	if (ret) {
		pr_warning("%s: read prop dynamic-sense-to-gnd fail\n", __func__);
	...
	if (gpio_is_valid(fsa_priv->reset_gpio)) {
		ret = devm_gpio_request_one(dev, fsa_priv->reset_gpio,GPIOF_OUT_INIT_LOW, "FSA4480_RST");
		pr_warning("%s: reset-gpio init low.\n", __func__);
	} else {
		pr_warning("%s: reset-gpio in dt node is missing\n", __func__);
	}
	//#endif /* OPLUS_ARCH_EXTENDS */
