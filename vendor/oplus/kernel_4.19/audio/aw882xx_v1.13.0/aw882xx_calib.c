...
static bool is_single_cali = true; /*if mutli_dev cali false, single dev true*/
...
static const char *cali_str[CALI_STR_MAX] = {"none", "start_cali", "cali_re",
	...
	"cali_q_f0", "show_q_f0", "get_re_range", "store_f0"
};
...
static unsigned int g_cali_f0_time_ms = AW_CALI_f0_DEFAULT_TIMER;
...
#define AWINIC_CALI_F0_FILE  "/mnt/vendor/persist/media/aw_cali_f0.bin"
#else
#define AWINIC_CALI_FILE  "/mnt/vendor/persist/factory/audio/aw_cali.bin"
#define AWINIC_CALI_F0_FILE  "/mnt/vendor/persist/factory/audio/aw_cali_f0.bin"
#endif /*OPLUS_ARCH_EXTENDS*/

#define AW_INT_DEC_DIGIT 10
#define AW_INT_DEC_DIGIT_F0 10
...
static int aw_cali_write_cali_f0_to_file(int32_t cali_f0, int channel)
{
	...
	pos = AW_INT_DEC_DIGIT_F0 * channel;
...
static int aw_cali_get_read_cali_f0(int32_t *cali_f0, int channel)
{
	struct file *fp = NULL;
	/*struct inode *node;*/
	int f_size;
	char *buf = NULL;
	int32_t int_cali_f0 = 0;
	loff_t pos = 0;
	mm_segment_t fs;
	char *re_buf = NULL;

	fp = filp_open(AWINIC_CALI_F0_FILE, O_RDONLY, 0);
	if (IS_ERR(fp)) {
		aw_pr_err("channel:%d open %s failed, error=%ld",
			channel, AWINIC_CALI_F0_FILE, PTR_ERR(fp));
		return -EINVAL;
	...
	if (kstrtoint(re_buf, 10, &int_cali_f0) == 0)
		*cali_f0 = int_cali_f0;
	else
		*cali_f0 = AW_ERRO_CALI_VALUE;

	re_buf = NULL;
	aw_pr_info("channel:%d buf:%s int_cali_f0: %d",
		channel, buf, int_cali_f0);
...
/*custom need add to set/get cali_f0 form/to nv*/
int aw_cali_write_f0_to_nvram(int32_t cali_f0, int32_t channel)
{
#ifdef AW_CALI_STORE_EXAMPLE
	if (channel >= AW_DEV_CH_MAX) {
		aw_pr_err("unsupported channel [%d]", channel);
...
int aw882xx_cali_read_f0_from_nvram(int32_t *cali_f0, int32_t channel)
{
	/*custom add, if success return value is 0 , else -1*/
#ifdef AW_CALI_STORE_EXAMPLE
	if (channel >= AW_DEV_CH_MAX) {
	...
	return aw_cali_get_read_cali_f0(cali_f0, channel);
#else
	return 0;
#endif
}
...
int aw_cali_store_cali_f0(struct aw_device *aw_dev, int32_t f0)
{
	int ret = 0;

	aw_dev->cali_desc.cali_f0 = f0;
	ret = aw_cali_write_f0_to_nvram(f0, aw_dev->channel);
	if (ret) {
		aw_dev_err(aw_dev->dev, "write f0 to nvram failed!");
		goto store_cali_re_failed;
	}
	aw_dev_info(aw_dev->dev, "store cali f0 is %d", f0);
...
static int aw_cali_svc_dev_get_f0(struct aw_device *aw_dev)
{
	...
	aw_dev_info(aw_dev->dev, "cali f0 is %d", aw_dev->cali_desc.cali_f0);
...
static int aw_cali_svc_dev_get_f0_q(struct aw_device *aw_dev)
{
	...
	aw_dev_info(aw_dev->dev, "cali f0 is %d, q is %d",
...
int aw_cali_svc_set_devs_f0_str(struct aw_device *aw_dev, const char *re_str)
{
	int ret, cnt = 0;
	struct list_head *dev_list, *pos = NULL;
	struct aw_device *local_dev = NULL;
	int f0_data[AW_DEV_CH_MAX] = {0};
	...
	ret = sscanf(re_str, "pri_l:%d pri_r:%d sec_l:%d sec_r:%d tert_l:%d tert_r:%d quat_l:%d quat_r:%d",
					&f0_data[AW_DEV_CH_PRI_L],
					&f0_data[AW_DEV_CH_PRI_R],
					&f0_data[AW_DEV_CH_SEC_L],
					&f0_data[AW_DEV_CH_SEC_R],
					&f0_data[AW_DEV_CH_TERT_L],
					&f0_data[AW_DEV_CH_TERT_R],
					&f0_data[AW_DEV_CH_QUAT_L],
					&f0_data[AW_DEV_CH_QUAT_R]);
	...
	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		if (local_dev->channel < AW_DEV_CH_MAX) {
			ret = aw_cali_store_cali_f0(local_dev, f0_data[local_dev->channel]);
			if (ret < 0)
				return ret;
			cnt++;
		}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = sscanf(buf, "%d %d", &time[0], &time[1]);
	if (ret <= 0) {
	...
	if (ret >= 1) {
		...
		g_cali_re_time = time[0];
		if (ret == 2) {
			g_cali_f0_time_ms = time[1];
		}

	}
	aw_dev_info(aw_dev->dev, "re time:%d, f0 time %d",
						g_cali_re_time, g_cali_f0_time_ms);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (is_single_cali) {
		...
		ret = aw_cali_store_cali_f0(aw_dev, data);
		if (ret < 0) {
			aw_dev_err(aw_dev->dev, "set f0 %d failed", data);
			return -EPERM;
		}
	} else {
		ret = aw_cali_svc_set_devs_f0_str(aw_dev, buf);
		if (ret <= 0) {
			aw_dev_err(aw_dev->dev, "set f0 str %s failed", buf);
			return -EPERM;
		}
	}
...
/*calib and show re*/
static ssize_t oplus_cali_re_f0_show(struct device *dev, char *buf, int cali_cmd)
{
	int ret = 0;
	int i = 0;
	struct aw882xx *aw882xx = dev_get_drvdata(dev);
	...
	/* start calib */
	if (cali_cmd == AW_CALI_CMD_RE_F0) {
		ret = aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_RE_F0, is_single_cali, CALI_OPS_HMUTE|CALI_OPS_NOISE);
	} else if (cali_cmd == AW_CALI_CMD_RE) {
		ret = aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_RE, is_single_cali, CALI_OPS_HMUTE);
	}
	if (ret < 0) {
		len += snprintf(buf+len, PAGE_SIZE-len, "cali Re cmd failed\n");
...
ssize_t oplus_aw_cali_re_f0(struct device *dev, char *buf)
{
	...
	if (aw882xx->need_f0_cali) {
		pr_info("[awinic] %s: cali f0 and re.\n",
			__func__);
		return oplus_cali_re_f0_show(dev, buf, AW_CALI_CMD_RE_F0);
	} else {
		pr_info("[awinic] %s: cali re.\n",
				__func__);
...
int aw882xx_load_re_for_test(int32_t chan, int32_t *re)
{
	...
	ret = aw882xx_cali_read_re_from_nvram(&cali_re, chan);
	if (ret < 0) {
				...
				struct class_attribute *attr, const char *buf, size_t len)
{
	...
	ret = aw_cali_svc_set_devs_f0_str(local_dev, buf);
	if (ret <= 0) {
		aw_dev_err(local_dev->dev, "set f0 str %s failed", buf);
		return -EPERM;
	}
...
static ssize_t aw_cali_misc_write(struct file *filp, const char __user *buf, size_t size, loff_t *pos)
{
	...
	case CALI_STR_SET_F0: {
		kernel_buf[size] = '\0';
		ret = aw_cali_svc_set_devs_f0_str(aw_dev,
				kernel_buf + strlen(cali_str[CALI_STR_SET_F0]) + 1);
	}break;
...
int aw_cali_parse_re_dt(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		...
		aw_dev_info(aw_dev->dev, "read aw-f0-min failed, use default");
	}

	ret = of_property_read_u32(aw_dev->dev->of_node, "aw-f0-max", &aw_dev->f0_max);
	if (ret < 0) {
		aw_dev->f0_max = AW_CALI_F0_DEFAULT_MAX;
		aw_dev_info(aw_dev->dev, "read aw-f0-max failed, use default");
	}

	if (aw_dev->f0_min >= aw_dev->f0_max) {
