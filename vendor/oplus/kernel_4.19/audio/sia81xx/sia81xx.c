...
#define SIA8001_OWI_MODE				(11)
#define SIA8102_OWI_MODE				(9)
...
struct chip_owi_table {
	int owiValIndex[AUDIO_SCENE_NUM];
};

static struct chip_owi_table g_chip_owi_table[CHIP_TYPE_INVALID] = {
	[CHIP_TYPE_SIA8102].owiValIndex = {
		[AUDIO_SCENE_PLAYBACK] = SIA8102_OWI_MODE,
		[AUDIO_SCENE_VOICE]    = DEFAULT_OWI_MODE,
	},
	[CHIP_TYPE_SIA8001].owiValIndex = {
		[AUDIO_SCENE_PLAYBACK] = SIA8001_OWI_MODE,
		[AUDIO_SCENE_VOICE]    = DEFAULT_OWI_MODE,
	},
};
...
void distinguish_chip_type_v2(sia81xx_dev_t *sia81xx)
{
	...
	if (CHIP_TYPE_UNKNOWN == sia81xx->chip_type) {
		...
		if (1 == sia81xx->en_dyn_id) {
			if (1 == check_sia81xx_status(sia81xx)) {
				sia81xx->chip_type = CHIP_TYPE_SIA8001;
			} else if (0 == check_sia81xx_status(sia81xx)) {
				sia81xx->chip_type = CHIP_TYPE_SIA8102;
			}

			pr_info("[ info][%s] %s: sipa device is available, chip_type = %d .\r\n",
				LOG_FLAG, __func__, sia81xx->chip_type);
		} else {
			if (0 != check_sia81xx_status(sia81xx)) {
				sia81xx->chip_type = CHIP_TYPE_UNKNOWN;
				pr_info("[ info][%s] %s: there is no sipa device \r\n",
					LOG_FLAG, __func__);
			} else {
				pr_info("[ info][%s] %s: sipa device is available, chip_type = %d .\r\n",
					LOG_FLAG, __func__, sia81xx->chip_type);
	...
	struct sia81xx_dev_s *sia81xx)
{
	...
	if(0 == sia81xx->disable_pin && sia81xx->is_sia_pa) {
		if (sia81xx->en_dyn_id == 1) {
			sia81xx->owi_cur_mode = g_chip_owi_table[sia81xx->chip_type].owiValIndex[sia81xx->scene];
		}
...
/* Liang.Huang@PSW.MULTIMEDIA.AUDIODRIVER.MACHINE,2020/12/14 Add for sts test fail */
	...
	/*#Fuhua.wang@MULTIMEDIA.AUDIODRIVER,2021/02/04,check rst pin is on if need to set boost,otherwise i2c will err*/
	int rstpin_is_on = 0;
	/* Duan.Zengchao@PSW.MULTIMEDIA.AUDIODRIVER.MACHINE,2023/12/29 useless for 8102/8102s/8001 */
	if (g_sia81xx->en_dyn_id == 1) {
		return 0;
	}
	if (g_sia81xx->disable_pin == 0) {
		rstpin_is_on = gpio_get_value(g_sia81xx->rst_pin);
...
#endif /* OPLUS_BUG_COMPATIBILITY */
	/* Miao.Hu@MULTIMEDIA.AUDIODRIVER.AUDIODRIVER, 2023/02/22, sia8159 does not need boost */
...
static int sia81xx_probe(struct platform_device *pdev)
{
	...
	unsigned int val = 0;
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
	/*Miao.Hu@MULTIMEDIA.AUDIODRIVER.CODEC,2023/3/02,add PA manager*/
	struct oplus_speaker_device *speaker_device = NULL;
#endif /* CONFIG_SND_SOC_OPLUS_PA_MANAGER */
...
/* Kun.Zhao@MULTIMEDIA.AUDIODRIVER.SMARTPA, 2020/11/19, set or get boost voltage for sia8109 */
	...
	if(0 == disable_pin) {
		...
		/* segment of below code won't run if en_dynamic_id is NULL */
		if (1 == en_dyn_id) {
			ret = of_property_read_u32(pdev->dev.of_node, "sia8102s,playback_owi_mode", &val);
			g_chip_owi_table[CHIP_TYPE_SIA8102].owiValIndex[AUDIO_SCENE_PLAYBACK] = (int)val;
			pr_info("[ info][%s] %s: line = %d sia8102s,playback_owi_mode = %d\r\n",
					LOG_FLAG, __func__, __LINE__, val);
			if(0 != ret) {
				pr_err("[  err][%s] %s: get sia8102s,playback_owi_mode "
					"form dts fail, ret = %d !!! \r\n",
					LOG_FLAG, __func__, val);
			}
			ret = of_property_read_u32(pdev->dev.of_node, "sia8102s,voice_owi_mode", &val);
			g_chip_owi_table[CHIP_TYPE_SIA8102].owiValIndex[AUDIO_SCENE_VOICE] = (int)val;
			pr_info("[ info][%s] %s: line = %d sia8102s,voice_owi_mode = %d\r\n",
					LOG_FLAG, __func__, __LINE__, val);
			if(0 != ret) {
				pr_err("[  err][%s] %s: get sia8102s,voice_owi_mode "
					"form dts fail, ret = %d !!! \r\n",
					LOG_FLAG, __func__, val);
			}
			ret = of_property_read_u32(pdev->dev.of_node, "sia8001,playback_owi_mode", &val);
			pr_info("[ info][%s] %s: line = %d sia8001,playback_owi_mode = %d\r\n",
					LOG_FLAG, __func__, __LINE__, val);
			g_chip_owi_table[CHIP_TYPE_SIA8001].owiValIndex[AUDIO_SCENE_PLAYBACK] = (int)val;
			if(0 != ret) {
				pr_err("[  err][%s] %s: get sia8001,playback_owi_mode "
					"form dts fail, ret = %d !!! \r\n",
					LOG_FLAG, __func__, val);
			}
			ret = of_property_read_u32(pdev->dev.of_node, "sia8001,voice_owi_mode", &val);
			pr_info("[ info][%s] %s: line = %d sia8001,voice_owi_mode = %d\r\n",
					LOG_FLAG, __func__, __LINE__, val);
			g_chip_owi_table[CHIP_TYPE_SIA8001].owiValIndex[AUDIO_SCENE_VOICE] = (int)val;
			if(0 != ret) {
				pr_err("[  err][%s] %s: get sia8001,voice_owi_mode "
					"form dts fail, ret = %d !!! \r\n",
					LOG_FLAG, __func__, val);
			}
			...
			pr_info("[ info][%s]  %s: line = %d  id_pin  = %d\r\n", LOG_FLAG, __func__, __LINE__, id_pin);
			if (id_pin < 0) {
...
/* Wenyang.Fan@PSW.MULTIMEDIA.AUDIODRIVER.MACHINE,2020/06/16, Add for adjust codec & Sia PA sequence */
	...
	if (1 == sia81xx->en_dyn_id) {
		distinguish_chip_type_v2(sia81xx);
	}
...
static int sia81xx_remove(struct platform_device *pdev)
{
	...
	pr_info("[ info][%s] %s: remove \r\n", LOG_FLAG, __func__);
#ifdef CONFIG_SND_SOC_OPLUS_PA_MANAGER
	/*Miao.Hu@MULTIMEDIA.AUDIODRIVER.CODEC,2023/03/02,add PA manager*/
	oplus_speaker_pa_remove(oplus_pa_sia_node);
#endif /* CONFIG_SND_SOC_OPLUS_PA_MANAGER */
