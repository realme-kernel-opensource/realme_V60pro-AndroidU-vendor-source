...
// add for sia8157
	ret = of_property_read_u32(pdev->dev.of_node, "en_spk_sia8157", &en_spk_sia8157);
	if(0 != ret) {
		pr_info("[ info][%s] %s:, en_spk_sia8157 = %d\r\n",
			LOG_FLAG, __func__, en_spk_sia8157);
		en_spk_sia8157 = 0;
	} else {
		pr_info("[ info][%s] %s:, en_spk_sia8157 = %d\r\n",
			LOG_FLAG, __func__, en_spk_sia8157);
